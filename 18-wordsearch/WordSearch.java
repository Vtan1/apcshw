...
public class WordSearch{
	...
	int orig = col;
	boolean work = true;
	if (c+w.length()<=board[r].length && r < board.length && c < board[r].length && r > 0 && c > 0){
	...
	int orig2 = col;
	boolean work = true;
	if (r-w.length()>=-1 && r > 0 && c > 0 && r+w.length()<=board.length && r < board.length && c < board[r].length){
	    for (int i=0;i<w.length();i++){
		if (work) {
		    if (board[r][c] == '.') {
			//board[r][c] = w.charAt(i);
			...
			if (!(board[r][c] == w.charAt(i))) {
			    System.out.println(w + " encountered a different word while going backwards diagonally.");
			    work = false;
			} else {
			    r++;
			    c--;
	...
	//try {
	    ...
	    w.addWordHR("hello",10,555);
	    w.addWordVD("howdy",15,1);
	    w.addWordDDL("swag",11,3);
	    System.out.println(w);
	    //} catch (Exception e) {
	    //System.out.println("One of your parameters is out of bounds.");
    }
}
